...
public class Pawn extends AbstractPiece implements Piece {
  ...
  boolean wouldMoveOffBoardVertically(Coordinates from, Board board){
  ...
  public List<Move> getAllowedMoves(Coordinates from, Board board) {
    ...
    if ((getColour().equals(PlayerColour.WHITE))) {
      if (isOnBoard(from, board) && (!wouldMoveOffBoardVertically(from, board)) && (board.get(from.plus(-1,0)) == null)) {
      ...
      if (isOnBoard(from, board) && (!wouldMoveOffBoardVertically(from, board)) && (board.get(from.plus(-1,-1))) != null && board.get(from.plus(-1,-1)).getColour().equals(PlayerColour.BLACK)) {
      ...
      if (isOnBoard(from, board) && (!wouldMoveOffBoardVertically(from, board)) && (board.get(from.plus(1,-1))) != null && board.get(from.plus(1,-1)).getColour().equals(PlayerColour.WHITE)) {
        allowedMoves.add(new Move(from, from.plus(1, -1)));
      }
      if (isOnBoard(from, board) && (!wouldMoveOffBoardVertically(from, board)) && (board.get(from.plus(1,1))) != null && board.get(from.plus(1,1)).getColour().equals(PlayerColour.WHITE)) {
        allowedMoves.add(new Move(from, from.plus(1, 1)));
